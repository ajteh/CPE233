...
SEI
MAIN: 	CMP R6, 0x00    ;Main Subroutine allows the note to be played      
		BREQ MAIN
		CMP R7,	0x09	;Also keeps the Recording from holding the last note
		BRCS MAIN
		OUT R6,SPEAKER
		BRN MAIN	
				
LOOP1: 	MOV R4,255		;Delay Loops
...
PLAYRECORDING:
 	CMP R15,R16     ; if R15 equals R16, nothing is played, deals with case of no recording or deleting
...
INTERRUPT:	
		IN R6, 	KEYBOARD	;Sets R6 to whatever comes in in the input from the interrupt 
		CMP R6,	0X00
		BREQ	RETURN
